...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
		...
		// Make the query:
		$q = "DELETE FROM messages WHERE user_id=$id LIMIT 1";
	...
	$q = "SELECT CONCAT(first_name, ', ', last_name) FROM messages WHERE user_id=$id";
	$r = @mysqli_query($dbc, $q);
	...
	if (mysqli_num_rows($r) == 1) { // Valid user ID, show the form.
		...
		echo "<h3>Message From: $row[0]</h3>
		Are you sure you want to delete message from this user?";
